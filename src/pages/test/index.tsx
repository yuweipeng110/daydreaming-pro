...
import { CalleArr } from '@/utils/func';
...
// };
// CalleArr([mockData]);
// colors对象转数组 const colorsArr = Object.keys(obj);
// 取出随机colors中的一项 const colorsIndex = Math.floor((Math.random()*arr_wx.length)); colorsArr[colorsIndex]
// 递归循环data中children 增加status属性为colorsArr[colorsIndex]
...
export default () => {
  ...
  useEffect(() => {
    ...
    graph.on('name-shape:click', (evt: any) => {
      ...
      console.log(`/scene/rule/update?id=${branchId}&app_id=${appId}&scene_id=${sceneId}`);
    });
    graph.data(data);
    graph.render();
